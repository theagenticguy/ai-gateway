...
| `DynamoDB` | Infra plane | `gateway-usage` (hash=`scope_id`, range=`period_date`), `gateway-budgets` (hash=`budget_id`, range=`scope`, plus the `scope-index` GSI), and config tables | `infrastructure/main.tf:259-275`; ops in `src/*/handler.py` |
