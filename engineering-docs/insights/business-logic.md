...
| `tier` must be one of sandbox/standard/high/unlimited (defaults to STANDARD; canonical enum in `gwcore.tiers`) | team_registration | `src/gwcore/tiers.py:21`, `src/team_registration/models.py:35` | Reject 400 |
...
- **Tier budget defaults (single source of truth, issue #260):** the canonical tier table lives in `gwcore.tiers.TIER_DEFAULTS` (SANDBOX=$25, STANDARD=$100, HIGH=$1000, UNLIMITED=$10000 monthly; unknown tier falls back to STANDARD). `src/gwcore/tiers.py:32`. On registration a team's monthly budget is seeded from it via `TIER_BUDGET_DEFAULTS`, written in the real budgets-table schema so enforcement and budget_admin both see it. `src/team_registration/models.py:24`; seeded at `src/team_registration/routes.py:155-170`.
- **Budget-enforcement tier defaults (same table):** the enforcement Lambda derives its built-in defaults from `gwcore.tiers.TIER_DEFAULTS`, overridable via the `TIER_DEFAULTS` env var (JSON) or per-team DynamoDB budget records. `src/budget_enforcement/handler.py:52,66-79`.
