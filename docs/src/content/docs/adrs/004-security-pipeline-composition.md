---
title: "ADR-004: 3-Phase Container Security Pipeline"
description: Structured security pipeline with hadolint + checkov (pre-build), trivy + syft (post-build), and cosign (post-scan).
sidebar:
  order: 4
---
...
Skip grype (trivy covers container scanning) and osv-scanner (`uv audit` provides native OSV scanning).
