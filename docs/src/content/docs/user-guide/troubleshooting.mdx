...
1. Test a different provider to isolate the issue:
   ```bash
   # Send a request for a different model to exercise the fallback chain
...
## No Provider Header Needed

If you are migrating from an earlier (Portkey-based) release, you may have configured an `x-portkey-provider` header on every request. agentgateway selects the provider server-side from its rendered config, so this header is gone -- there is no longer a `{"error": "provider is not set"}` failure mode.

Remove any leftover `x-portkey-*` headers from your agent configuration:

<Tabs>
<TabItem label="Claude Code">


Unset `ANTHROPIC_CUSTOM_HEADERS` if it carries a provider header -- it is no longer needed:
```bash
...
Remove any `options.headers` block from `opencode.json`. Only `baseURL` is required:
```json
"options": {
...
Goose needs only `GOOSE_PROVIDER`, `OPENAI_HOST`, and `OPENAI_API_KEY`. Unset any leftover header var:
```bash
...
Remove the `requestOptions.headers` block from `~/.continue/config.yaml`. Only `apiBase`, `apiKey`, and `model` are required.

</TabItem>
<TabItem label="LangChain">


Drop `default_headers` from `ChatOpenAI` -- pass only `base_url`, `api_key`, and `model`:
```python
llm = ChatOpenAI(
...
Remove the `[model_providers.gateway.headers]` section from `~/.codex/config.toml`. Only `name`, `base_url`, and `env_key` are required.
