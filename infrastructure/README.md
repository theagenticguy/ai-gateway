...
| <a name="input_default_routing_strategy"></a> [default\_routing\_strategy](#input\_default\_routing\_strategy) | Default routing strategy for the gateway. Options: single (one provider), fallback (try next on failure), loadbalance (weighted distribution). | `string` | `"single"` | no |
