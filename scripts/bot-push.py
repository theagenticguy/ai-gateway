...
def preflight(py_files: list[str]) -> None:
    """Run ruff lint + format check on changed Python files."""
    if not py_files:
        return
    print("Pre-flight: ruff check...")
    ruff = shutil.which("ruff") or shutil.which("uvx")
    if not ruff:
        print("  Warning: ruff not found, skipping pre-flight checks")
        return
    cmd = [ruff] if "ruff" in ruff else [ruff, "ruff"]
    result = subprocess.run(  # noqa: S603
        [*cmd, "check", *py_files],
        capture_output=True,
        text=True,
    ...
    if result.returncode:
        print(result.stdout)
        print("Pre-flight lint failed. Run 'uvx ruff check --fix .' to fix.")
        sys.exit(1)
    result = subprocess.run(  # noqa: S603
        [*cmd, "format", "--check", *py_files],
        capture_output=True,
        text=True,
        check=False,
    )
    if result.returncode:
        print(result.stdout)
        print("Pre-flight format failed. Run 'uvx ruff format .' to fix.")
        sys.exit(1)
    print("  Passed")
...
def main() -> None:
    ...
    parser.add_argument("--no-check", action="store_true", help="Skip pre-flight lint/format")
    ...
    # Pre-flight lint/format on changed Python files
    if not args.no_check:
        py_files = [p for s, p in changes if s != "D" and p.endswith(".py")]
        preflight(py_files)

    ctx = {"repo": repo, "token": token, "base_sha": base_sha, "base_tree_sha": base_tree_sha, "api_base": api_base}
    commit_sha = push_changes(ctx, args, changes)
    print(f"Done! ({commit_sha[:8]})")


def push_changes(ctx: dict, args: argparse.Namespace, changes: list[tuple[str, str]]) -> str:
    """Create blobs, tree, commit, branch, and optionally a PR."""
    repo, token = ctx["repo"], ctx["token"]
    base_sha, base_tree_sha, api_base = ctx["base_sha"], ctx["base_tree_sha"], ctx["api_base"]
    ...
    commit = gh(
        ...
        "git/commits",
        token,
        json={"message": args.message, "tree": tree["sha"], "parents": [base_sha]},
    )
    ...
    if refs:
        gh("patch", repo, f"git/refs/heads/{args.branch}", token, json={"sha": commit["sha"], "force": True})
