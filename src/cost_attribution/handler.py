...
import base64, gzip, json, logging, os
...
if not logger.handlers:
    ...
    _h.setFormatter(logging.Formatter(json.dumps({"timestamp":"%(asctime)s","level":"%(levelname)s","logger":"%(name)s","message":"%(message)s"})))
...
def _extract_provider(record: dict[str, Any]) -> str:
    ...
    return provider if provider else record.get("provider", "unknown")
...
def _extract_metrics(log_event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if total_tokens == 0 and (prompt_tokens + completion_tokens) == 0: return None
    if total_tokens == 0: total_tokens = prompt_tokens + completion_tokens
    provider = _extract_provider(record)
    model = record.get("model", "unknown")
    return {"provider": provider, "model": model, "prompt_tokens": prompt_tokens,
...
def handler(event: dict[str, Any], context: Any = None) -> dict[str, Any]:
    try: log_data = _decode_log_data(event)
    except Exception:
    ...
    for log_event in log_events:
        try:
            ...
            if m: extracted.append(m)
        except Exception:
            logger.exception("Failed to process log event: %s", log_event.get("id", "unknown"))
            errors += 1
    if extracted:
        try: _publish_metrics(extracted)
        except Exception:
            logger.exception("Failed to publish metrics to CloudWatch")
            return {"statusCode": 500, "processed": len(extracted), "errors": errors, "error": "Failed to publish metrics"}
    return {"statusCode": 200, "total_events": len(log_events), "processed": len(extracted),
            "skipped": len(log_events) - len(extracted) - errors, "errors": errors}
